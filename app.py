...
def contar_terminos(df: pd.DataFrame, columnas: list, terminos: list) -> dict:
    ...
    return {
        t: len(re.findall(r"\b" + re.escape(t) + r"\b", texto))
        for t in terminos
        if len(re.findall(r"\b" + re.escape(t) + r"\b", texto)) > 0
    }
...
def render_kpis(df: pd.DataFrame):
    ...
    for col, val, label in [
        (c1, f"{total:,}", "Artículos analizados"),
        (c2, f"{año_min} – {año_max}", "Rango temporal"),
        (c3, f"{citas_total:,}", "Citas acumuladas"),
        (c4, str(prom_citas), "Citas promedio"),
        (c5, f"{autores_unicos:,}", "Autores únicos"),
    ]:
        with col:
...
def render_sidebar(df: pd.DataFrame):
    with st.sidebar:
        ...
        if "year" in df.columns:
            ...
            if len(años) >= 2:
                rango = st.slider("📅 Rango de años", int(min(años)), int(max(años)),
...
def grafico_evolucion_temporal(df: pd.DataFrame):
    if "year" not in df.columns:
        st.warning("No se encontró columna de año."); return
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=pub["year"], y=pub["publicaciones"],
                         name="Publicaciones/año", marker_color="#0ea5e9", opacity=0.7))
    fig.add_trace(go.Scatter(x=pub["year"], y=pub["tendencia"],
    ...
    st.markdown(f"""<div class="insight-box">
        <strong>📊 Insight:</strong> El pico de publicaciones se alcanza en <strong>{int(peak['year'])}</strong>
        con <strong>{int(peak['publicaciones'])}</strong> artículos. La tendencia ascendente confirma que
        este campo está en plena expansión y es considerado vigente por la comunidad científica.
    </div>""", unsafe_allow_html=True)


def grafico_autores_citados(df: pd.DataFrame, top_n: int = 15):
    if "authors" not in df.columns or "cites" not in df.columns:
        st.warning("Se requieren columnas de autores y citas."); return
    df2 = df.copy()
    df2["primer_autor"] = df2["authors"].apply(extraer_primer_autor)
    ranking = (df2.groupby("primer_autor")["cites"].sum()
               .reset_index().sort_values("cites", ascending=False).head(top_n))
    ranking = ranking[ranking["primer_autor"] != "Desconocido"]
    fig = px.bar(ranking.sort_values("cites"), x="cites", y="primer_autor", orientation="h",
                 color="cites", color_continuous_scale=["#1e3a5f", "#0ea5e9", "#38bdf8"],
    ...
    top = ranking.iloc[0]
    st.markdown(f"""<div class="insight-box">
        <strong>🏆 Insight:</strong> <strong>{top['primer_autor']}</strong> lidera con
        <strong>{int(top['cites']):,} citas</strong>, siendo el referente más influyente del corpus.
...
def grafico_palabras_clave(df: pd.DataFrame):
    ...
    if not conteos:
        st.warning("No se encontraron columnas de texto para analizar."); return
    freq = (pd.DataFrame(list(conteos.items()), columns=["Término", "Frecuencia"])
            .sort_values("Frecuencia", ascending=False).head(25))

    def cat(t):
        if t in TERMINOS_IA: return "🤖 IA & Data Science"
        if t in TERMINOS_NEGOCIOS: return "💼 Negocios"
        return "👔 Empleabilidad"

    freq["Categoría"] = freq["Término"].apply(cat)
    color_map = {"🤖 IA & Data Science": "#38bdf8", "💼 Negocios": "#a78bfa", "👔 Empleabilidad": "#34d399"}
    fig = px.bar(freq, x="Frecuencia", y="Término", orientation="h", color="Categoría",
    ...
    st.markdown(f"""<div class="insight-box">
        <strong>🔍 Insight:</strong> El término <strong>"{top_term}"</strong> domina el corpus textual.
        La co-presencia de términos de empleabilidad junto a IA/Data Science en abstracts confirma
...
def grafico_relevancia_temporal(df: pd.DataFrame):
    if "year" not in df.columns or "cites" not in df.columns:
        st.warning("Se requieren columnas de año y citas."); return
    df_plot = df[df["cites"] > 0].copy()
    if df_plot.empty:
        st.info("No hay artículos con citas registradas."); return
    color_col = "doc_type" if "doc_type" in df_plot.columns else None
    fig = px.scatter(df_plot, x="year", y="cites", color=color_col,
    ...
    fig.add_trace(go.Scatter(x=avg["year"], y=avg["cites"], mode="lines",
                             name="Promedio por año",
                             line=dict(color="#f59e0b", width=2, dash="dot")))
    for _, row in df_plot.nlargest(3, "cites").iterrows():
        label = str(row.get("title", ""))[:35] + "…"
        fig.add_annotation(x=row["year"], y=row["cites"], text=label,
    ...
    st.plotly_chart(fig, use_container_width=True)
    media = round(df_plot["cites"].mean(), 1)
    maximo = int(df_plot["cites"].max())
    st.markdown(f"""<div class="insight-box">
        <strong>💡 Insight:</strong> Promedio global de <strong>{media} citas/artículo</strong> y un máximo de
        <strong>{maximo:,} citas</strong>. Los artículos fundacionales acumulan más citas históricas,
        mientras las publicaciones recientes (2020+) muestran cre
